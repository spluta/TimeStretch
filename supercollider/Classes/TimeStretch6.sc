...
//
// 		{|array|
// 			var temp = Array.newClear(array.size);
// 			var hannIn = Signal.hanningWindow(512).copyRange(0, 255);
// 			var hannOut = Signal.hanningWindow(2048).copyRange(0, 1024);
// 			var mem = Array.fill(10, {0});
// 			var count=1;
//
// 			array.do{|item, i|
//
// 				if(item.abs>=0.0001)
// 				{
// 					mem = mem.put(i%mem.size, 1);
// 				}{
// 					mem = mem.put(i%mem.size, 0);
// 				};
// 				//mem.postln;
// 				if(mem.sum>0){
// 					temp.put(i, 1);
...
// 			};
//
// 			temp.do{|item, i| if((item==1)and:(temp[i-1]==0)){((hannIn.size-1)..0).do{|num, index| temp = temp.put(i-num, hannIn[index])}}};
// 			temp = temp.reverse;
// 			temp.do{|item, i| if(i>1){if((item==1)and:(temp[i-1]<1)){((hannOut.size-1)..0).do{|num, index| temp = temp.put(i-num,
// 				max(temp[i-num],hannOut[index])
// 			)}}}};
// 			temp = temp.reverse;
// 		}
//
...
// 	}
//
// 	*processChunk {|server, tempDir, floatArray, chanNum, outFolder, numSplits, maxWindowSize, durMult, chunkSize, frameChunks, fCNum, lastArrayA, serverNum, fftType=0, binShift = 0|
// 		var frameChunk = frameChunks[fCNum];
// 		var writeFile;
// 		var bigList = /*List.fill(chunkSize+(maxWindowSize*durMult), {0});*/List.fill(chunkSize, {0});
// 		var windowSizes, binShiftSamples;
//
...
// 				"next Chunk".postln;
//
// 				this.processChunk(server, tempDir, floatArray, chanNum, outFolder, numSplits, maxWindowSize, durMult, chunkSize, frameChunks, fCNum, lastArrayA, serverNum, fftType, binShift);
// 			}{
// 				"doneWChannel".postln;
...
// 	}
//
// 	*stretch {|inFile, outFolder, dur, durMult=100, chanArray, chunkSize = -1, numSplits = 9, startFrame=0, fftType=0, winType=0, binShift=0, merge=0|
...
// 					this.processChunk(server, tempDir, floatArray, chanNum, outFolder, numSplits, maxWindowSize, durMult, chunkSize, frameChunks, startFrame, lastArrayA, serverNum, fftType, binShift);
// 				});
// 			});
//
...
// 							this.processChunk(server, tempDir, floatArray, chanNum, outFolder, numSplits, maxWindowSize, durMult, chunkSize, frameChunks, 0, lastArrayA, serverNum, fftType, binShift);
// 						});
// 					});
// 				}{
// 					"we're done".postln;

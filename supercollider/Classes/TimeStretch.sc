...
TimeStretch {
	...
	*stretchNRT { |inFile, outFile, durMult, fftMax = 65536, overlaps = 2, numSplits = 9, amp = 1, action, verbosity=0|
	...
	*addBundles {|nrtJam, server, inFile, buffer, chanNum, durMult, overlaps, pan, amp, filtVals, fftVals, fftMax|
		...
		filtVals.do{|fv, i|
			...
			nrtJam.add([0.0, Synth.basicNew(("pb_monoStretch_Overlap"++overlaps[i]), server).newMsg(args: [bufnum: buffer.bufnum, pan: pan, fftSize:fftVals[i], fftMax:fftMax, \stretch, durMult, \hiPass, fv[0], \lowPass, fv[1]-1, \amp, amp])])
		};
		^nrtJam
	}

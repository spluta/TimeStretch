+ TimeStretch {
	...
	*mergeResonAndTrans {|inFile, transientLevel=0.707, numChans=2|
					...
					buffers.do{|filesArray, fileNum|
						filesArray.do{|chan, i|
							chan.do{|buf, i2|
								...
								FluidBufCompose.processBlocking(server, buf, 0, -1, 0, -1, [1,transientLevel].at(fileNum), finalBuf, i2*chunkSize, i, 1, true, {("chan:"+i+"buffer"+i2).postln}).wait;
								if(fileNum==0){

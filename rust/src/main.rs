...
#[command(about = "NessStretch Time Stretching Algorithm", long_about = None)]
...
struct Cli {
    ...
    ///Default is 0 - no cut. Setting to 1 cuts the fade in and fade out material from the output file, making the sound start immediately in the output file and shortening the file as well.
    #[arg(short, long, default_value_t = 1)]
    cut_fades: usize,
...
fn main() {
    ...
    //klask::run_app(app, Settings::default(), |matches| {
        ...
        let mut num_slices: usize = cli.slices;
        let verbosity = cli.verbosity;
        let cut_fades = cli.cut_fades;
